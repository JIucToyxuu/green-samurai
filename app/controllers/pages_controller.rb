class PagesController < ApplicationController
	...
	def index
		...
		if params[:q].nil?
			if current_user.nil?
				@courses = Course.all.order("LOWER(#{params[:sort_by]})")
			else
				@courses = Course.all.joins(:pupils).where.not('pupils.users_id' => current_user.id).order("LOWER(#{params[:sort_by]})")
				@all = Course.all.order("LOWER(#{params[:sort_by]})")
				@all.each do |course|
					if !course.pupils[0]
						@courses << course
					end
	...
	def courses
		...
		if !params[:q].nil?
			@universities = University.joins(:courses).includes(:courses).where("LOWER('courses'.'name') LIKE '%#{params[:q].downcase}%'")
		end
		@universities = @universities.all.joins(:courses).order('universities.id').includes(:courses).order("LOWER(courses.#{params[:sort_by]})")
		#category for sorting data
	...
	def university
		...
		@university = University.find_by_abbreviation(params[:university])
		@courses =  @university.courses.order("LOWER(#{params[:sort_by]})");
		current_user_courses
	end
